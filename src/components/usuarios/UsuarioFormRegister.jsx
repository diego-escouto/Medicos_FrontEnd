    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE_URL}/cliente`, {
                ...
                // Envia apenas os campos que o backend espera (nome, email, senha).
                // O reCAPTCHA é verificado no frontend (exige que esteja preenchido),
                // mas ainda não é enviado ao backend para evitar erro de validação.
                body: JSON.stringify({
                    nome,
                    email,
                    senha,
            ...
            if (!res.ok) {
                throw new Error(data?.message || data?.erro || "Falha no registro");
            }

            // O backend atual retorna { token: '...' } após criar o cliente.
            const at = data?.token || data?.access_token;
            if (!at) throw new Error("Resposta sem token de autenticação");
