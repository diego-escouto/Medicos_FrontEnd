...
const UsuariosFormLogin = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE_URL}/cliente/login`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                credentials: "include", // recebe/enviar cookies (refresh HttpOnly)
                // Envia apenas os campos que o backend espera (email, senha).
                body: JSON.stringify({
                    email,
                    senha,
                }),
            ...
            if (!res.ok) {
                // O backend do projeto usa a propriedade `message` ou `erro` em erros
                throw new Error(data?.message || data?.erro || "Falha no login");
            }

            // O backend atual retorna { token: '...' }
